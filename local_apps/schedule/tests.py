...
class ScheduleTest(TestCase):
    ...
    def test_days_of_week_some(self):
        self.rule(days_of_week=[SU, TU],
                  end_date=self.DATE + relativedelta(weeks=+1, days=-1))
        allocations = self.get_allocations()
        self.assertEqual(2, len(allocations))

    # special handling of long chores

    def test_biweekly_rule(self):
        self.rule(days=8, days_of_week=[TU],
                  end_date=self.DATE + relativedelta(weeks=+4, days=-1))
        allocations = self.get_allocations()
        self.assertEqual(2, len(allocations))
        dates = sorted(x.date for x in allocations)
        # 2001 starts on a monday
        self.assertEqual(
            [datetime.date(2001, 1, 2), datetime.date(2001, 1, 2 + 14)], dates)
    ...
    def get_allocations(self):
        ...
        for (date, days), quantity in raw_allocations.iteritems():
            ...
            for _i in xrange(quantity):
                allocations.append(
                    Alloc(chore=self.chore, date=date, days=days))
        return allocations
